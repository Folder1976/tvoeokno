...
$data['language_id'] = (int)$this->config->get('config_language_id');
		...
		//var_dump($data);
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/checkout/checkout.tpl')) {
			$this->template  = $template =  $this->config->get('config_template') . '/checkout/checkout.tpl';
		} else {
	...
	public function payment_method_validate(&$data  = array()) {
		...
		if (!$json) {
			...
			/*
			$this->request->post['payment_method'] = 'free_checkout';
...
*/
			...
			if (!$json) {
				$this->session->data['payment_method'] = 'free_checkout';//$this->session->data['payment_methods'][$this->request->post['payment_method']];
	...
	public function shipping_address($render = true, &$data  = array()) {
		...
		if ($render !== false)
		{
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/checkout/shipping_address.tpl')) {
				$this->template = $this->config->get('config_template') . '/checkout/shipping_address.tpl';
			} else {
				$this->template = '/checkout/shipping_address.tpl';
			}
	...
	public function shipping_method($render = true, &$data = array()) 
	{
		...
		if ($render !== false)	
		{
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/checkout/gn_shipping_method.tpl')) {
				$template = $this->template = $this->config->get('config_template') . '/checkout/gn_shipping_method.tpl';
			} else {
				$template = $this->template = '/checkout/gn_shipping_method.tpl';
			}
	...
	public function payment_address($render = true, &$data  = array()) {
		...
		if ($render !== false)	
		{
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/checkout/payment_address.tpl.tpl.tpl')) {
				$template =  $this->template = $this->config->get('config_template') . '/checkout/payment_address.tpl';
			} else {
				$template =  $this->template = '/checkout/payment_address.tpl';
			}
  	...
  	public function payment_method($render = true, &$data = array()) {
		...
		if ($render !== false)
		{
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/checkout/gn_payment_method.tpl'))
			{
				$template =  $this->template = $this->config->get('config_template') . '/checkout/gn_payment_method.tpl';
			} else {
				$template =  $this->template = '/checkout/gn_payment_method.tpl';
			}
	...
	public function checkout($render = true, &$data  = array()) 
	{
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/checkout/checkout.tpl')) {
			$this->template = $this->config->get('config_template') . '/checkout/checkout.tpl';
		} else {
  	...
  	public function guest($render = false, &$data  = array()) {
		...
		if ($render !== false)
		{
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/checkout/guest.tpl')) {
				$this->template = $this->config->get('config_template') . '/checkout/guest.tpl';
			} else {
				$this->template = '/checkout/guest.tpl';
			}
	...
	public function login($render = false, &$data  = array()) {
		...
		if ($render !== false)
		{
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/checkout/login.tpl')) {
				$this->template = $this->config->get('config_template') . '/checkout/login.tpl';
			} else {
				$this->template = '/checkout/login.tpl';
			}
...
//				$data['shipping_method'] = $this->request->post['shipping_method'];				
        ...
        $total_data = array(
			...
			*/
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/checkout/gn_cart.tpl')) {
				$template =  $this->template = $this->config->get('config_template') . '/checkout/gn_cart.tpl';
			} else {
				$template =  $this->template = '/checkout/gn_cart.tpl';
			}
		...
		if ($opencart2){
			...
			foreach ($this->cart->getProducts() as $product) {
                ...
                if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
                    ...
                    $price = $this->currency->format($unit_price, $this->session->data['currency'],$this->session->data['currency']);
                    $total = $this->currency->format($unit_price * $product['quantity'], $this->session->data['currency'],$this->session->data['currency']);
                } else {
                    $price = false;
                    $total = false;
                }
			...
			foreach ($total_data['totals'] as $total) {
				$data['totals'][] = array(
					'title' => $total['title'],
                    'text'  => $this->currency->format($total['value'], $this->session->data['currency'],$this->session->data['currency']),
				);
			}
			if ($render !== false)
			{
...
//						'key'                 => $product['key'],
			...
			if ($render !== false)
			{
				echo $this->data['payment'] = $this->getChild('extension/payment/' . $this->session->data['payment_method']['code']);
			}			
		}
  	}
}

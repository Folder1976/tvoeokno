...
class ControllerBlogBlog extends Controller { 
	...
	private function getList() {
		...
		foreach ($results as $result) {
			...
			$blog_category_ids = $this->model_blog_blog->getBlogCategories($result['blog_id']);
			$blog_category_id = array_shift($blog_category_ids);
			...
			$data['blogs'][] = array(
				...
				'category' => $this->model_blog_blog_category->getBlogCategory((int)$blog_category_id),
	...
	private function getForm() {
		...
		if (isset($this->request->post['show_date'])) {
			...
			$data['show_date'] = (int)$blog_info['show_date'];
		} else{
			$data['show_date'] = 1;
		}
